...
#include "Sensors/ds18b20_driver.hh"
...
void DS18B20Driver::read_all_temperatures()
{
	uint8_t i = 0;
	_device_count = 0;
	...
	_device_count = _bus->_num_roms;

	if(_device_count > 1)
	{
		for(i = 0; i < _bus->_num_roms; i++)
		{
			uint8_t *rom = _bus->_found_roms[i];

			start(rom);

			_temperatures[i] = {read_temperature_multiple(rom), rom_to_id(rom, sizeof(uint64_t))};
		}
	}
	else if(_device_count == 1)
	{
		uint8_t *rom = _bus->_found_roms[0];

		start_all();

		_temperatures[0] = {read_temperature_multiple(rom), rom_to_id(rom, sizeof(uint64_t))};
	}
	else
	{
	}
...
TemperatureMsg_t DS18B20Driver::get_temperature(size_t i)
{
	if(i < ONE_WIRE_SENSORS_MAX)
	{
		return _temperatures[i];
	}
	else{
		return TemperatureMsg_t{-1000.0, 0xFFFF};
	}
}

uint8_t DS18B20Driver::get_device_count()
{
