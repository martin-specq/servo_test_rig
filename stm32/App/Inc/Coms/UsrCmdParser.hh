...
private:
	...
	uint8_t _cmd_buf[BUFFER_SIZE] = { 0 };

public:
	UsrCmdParser(StreamInterface *stream) : _stream(stream) {}
	...
	void get_sin_params(float *angle_min_deg, float *angle_max_deg,
			float *period_s) {
	...
	void get_sin_sweep_params(float *angle_min_deg, float *angle_max_deg,
			float *period_min_s, float *period_max_s, uint32_t *n_periods,
		...
		memcpy((void*) n_cycles_per_period, (void*) &_cmd_buf[21],
				sizeof(uint32_t));
	}

	void get_trap_params(float *angle_min_deg, float *angle_max_deg,
...
private:
	size_t get_payload_from_cmd(uint8_t cmd) {
		...
		case CMD_SERVO_START_SIN_SWEEP:
			return 4 * sizeof(float) + 2 * sizeof(uint32_t);
		case CMD_SERVO_STOP:
			return 0;
		default:
			return 0;
